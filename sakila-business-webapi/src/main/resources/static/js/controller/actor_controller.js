...
App.controller('ActorController', ['$scope', 'ActorService', function ($scope, ActorService) {
        ...
        self.createActor = function (actor) {
            ActorService.createActor(actor).then(self.fetchAllActors, function (errResponse) {
                console.error('Error while creating Actor.');
            });
        };
        self.updateActor = function (actor) {
            ActorService.updateActor(actor).then(self.fetchAllActors, function (errResponse) {
                console.error('Error while updating Actor.');
            });
        };
        self.deleteActor = function (actorId) {
            ActorService.deleteActor(actorId).then(self.fetchAllActors, function (errResponse) {
